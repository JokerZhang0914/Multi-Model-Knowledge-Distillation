...
def get_args():
    parser = argparse.ArgumentParser(description="FCBFormer segmentation training")
    parser.add_argument("--seed", default=408, type=int)
    ...
    parser.add_argument("--lrs", default=True, type=str2bool, help="use ReduceLROnPlateau on val dice")
    parser.add_argument("--lrs_factor", default=0.5, type=float, help="scheduler factor")
    parser.add_argument("--lrs_min", default=1e-6, type=float, help="scheduler minimum lr")
    ...
    parser.add_argument(
        "--fcb_pretrained", 
        default="runs/seg_fcb/2026-0408-1605_csv_all_fcb/checkpoint/best_fcb.pth", 
        type=str, 
        help="optional checkpoint to initialize FCBFormer")
    parser.add_argument("--resume", default="", type=str, help="optional training checkpoint to resume")

    parser.add_argument("--work_dir", default="runs/seg_fcb", type=str)
...
class Optimizer:
    def __init__(self, args, device, writer):
        ...
        pvt_path = self._resolve_pvt_weight_path()
        self.model = self._build_fcbformer_with_pvt(pvt_path).to(self.device)
        logging.info(f"[*] PVT-B3 weights: {pvt_path}")

        pretrained_path = args.fcb_pretrained.strip()
        if pretrained_path:
            ...
            logging.info(
                f"[*] Loaded FCB pretrained: {pretrained_path} "
        ...
        if args.lrs:
            self.scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
                ...
                min_lr=args.lrs_min if args.lrs_min > 0 else 0.0,
            )
    ...
    def _resolve_pvt_weight_path(self) -> str:
        candidates = []
        if self.args.pvt_b3_weights.strip():
            candidates.append(resolve_path(self.args.pvt_b3_weights.strip()))
        candidates.extend(
            [
    ...
    def _build_fcbformer_with_pvt(self, pvt_weight_path: str):
        ...
        try:
            ...
            os.chdir(build_cwd)
            model = models.FCBFormer(size=self.args.trainsize)
        finally:
            os.chdir(old_cwd)
    ...
    def _load_model_state_dict(self, state_dict: Dict[str, torch.Tensor]) -> int:
        ...
        for key, value in state_dict.items():
            ...
            if nk in model_dict and model_dict[nk].shape == value.shape:
                matched[nk] = value
        if not matched:
            raise RuntimeError("No matched FCBFormer parameters found in checkpoint.")
    ...
    def train_one_epoch(self, epoch: int):
        ...
        for images, masks in pbar:
            ...
            with torch.autocast(self.device.type if self.device.type != "mps" else "cpu", enabled=self.use_amp):
                ...
                loss = self.dice_loss(logits, masks) + self.bce_loss(torch.sigmoid(logits), masks)
    ...
    def optimize(self):
        logging.info("[*] Start training FCBFormer ...")
        for epoch in range(self.start_epoch, self.args.epochs):
            ...
            if val_dice > self.best_dice:
                ...
                best_path = self._save_ckpt(epoch + 1, "best_fcb.pth")
                logging.info(f"[*] New best dice={self.best_dice:.4f}, saved: {best_path}")

            if (epoch + 1) % self.args.save_epochs == 0:
                save_path = self._save_ckpt(epoch + 1, f"fcb_epoch_{epoch + 1}.pth")
                logging.info(f"[*] Periodic checkpoint saved: {save_path}")

        final_path = self._save_ckpt(self.args.epochs, "last_fcb.pth")
...
def main():
    ...
    args.work_dir = os.path.join(base_work_dir, f"{timestamp}_{args.train_source}_{val_tag}_fcb")
    ...
    setup_logger(args.work_dir, "train_fcb.log")
