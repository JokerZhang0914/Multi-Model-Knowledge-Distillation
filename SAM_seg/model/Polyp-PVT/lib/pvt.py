...
class PolypPVT(nn.Module):
    def __init__(self, channel=32, pretrained_path=None):
        super(PolypPVT, self).__init__()

        self.backbone = pvt_v2_b2()  # [64, 128, 320, 512]
        if pretrained_path is None:
            root_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
            pretrained_path = os.path.join(root_dir, "pretrained_pth", "pvt_v2_b2.pth")

        if os.path.isfile(pretrained_path):
            save_model = torch.load(pretrained_path, map_location="cpu", weights_only=False)
            ...
            warnings.warn(
                f"PolypPVT pretrained file not found: {pretrained_path}. "
